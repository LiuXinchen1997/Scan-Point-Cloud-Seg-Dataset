...
class SurfacePointCloud:
    ...
    def get_sdf(self, query_points, use_depth_buffer=False, sample_count=11, return_gradients=False):
        if use_depth_buffer:
            ...
            face_ids = self.face_indices[indices[:, 0]]
        ...
        if return_gradients:
            ...
            return distances, gradients, face_ids
        else:
    ...
    def get_sdf_in_batches(self, query_points, use_depth_buffer=False, sample_count=11, batch_size=1000000,
        ...
        if return_gradients:
            ...
            face_ids = np.concatenate([batch[2] for batch in batches])
            return distances, gradients, face_ids
        else:
            distances = np.concatenate([batch[0] for batch in batches])
            face_ids = np.concatenate([batch[1] for batch in batches])
            return distances, face_ids
