...
from util.ipad_scaned import IPAD_SCANED
...
def main():
    ...
    IPAD_SCANED(split='train', data_root=args.data_root)
    IPAD_SCANED(split='val', data_root=args.data_root)
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    train_data = IPAD_SCANED(split='train', data_root=args.data_root, voxel_size=args.voxel_size, voxel_max=args.voxel_max, transform=train_transform, shuffle_index=True, loop=args.loop)
    ...
    if args.evaluate:
        val_transform = None
        val_data = IPAD_SCANED(split='val', data_root=args.data_root, voxel_size=args.voxel_size, voxel_max=800000, transform=val_transform)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.evaluate and (epoch_log % args.eval_freq == 0):
            loss_val, iou = validate(val_loader, model, criterion)
            if iou > best_iou:
                best_iou = iou
                is_best = True
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (coord, target, offset) in enumerate(train_loader):  # (n, 3), (n), (b)
        ...
        output_str = '*** output num of label '
        target_str = '*** target num of label '
        for i in range(2):
            output_str += '{}: {}, '.format(i, output[output == i].shape[0])
            target_str += '{}: {}, '.format(i, target[target == i].shape[0])
        output_str += ' ***'
        target_str += ' ***'
        logger.info(output_str)
        logger.info(target_str)
        
        iou = (torch.sum((output == 0) & (target == 0))) / (torch.sum((output == 0) | (target == 0)))
        iou += (torch.sum((output == 1) & (target == 1))) / (torch.sum((output == 1) | (target == 1)))
        iou /= 2.
...
def validate(val_loader, model, criterion):
    ...
    for i, (coord, target, offset) in enumerate(val_loader):
        data_time.update(time.time() - end)
        coord, target, offset = coord.cuda(non_blocking=True), target.cuda(non_blocking=True), offset.cuda(non_blocking=True)
        if target.shape[-1] == 1:
            target = target[:, 0]  # for cls
        ...
        iou += (torch.sum((output == 1) & (target == 1))) / (torch.sum((output == 1) | (target == 1)))
        iou /= 2.
        logger.info('*** test iou: {} ***'.format(iou))
        logger.info('*** output num of label 0: {}, label 1: {} ***'.format(output[output == 0].shape[0], output[output == 1].shape[0]))
